   ...
   return (
      <div key={user.id + user.fullName} className="p-3 rounded bg-gray-600 mb-2 flex justify-between">
         <div>
            ...
            {user.signUpDate && <div className='my-1 text-sm'>Since {user.signUpDateToString()}</div>}
            {user.phoneContact1 && <div>Contact 1: {user.phoneContact1}</div>}
            {user.phoneContact2 && <div>Contact 2: {user.phoneContact2}</div>}
         </div>
         {!user.master && (
            <ListItemEditDelete
               modalId={``}
